function [x, y] = positionEstimator_kalman(testData, modelParameters)
    ...
    % Initial estimates (now includes hidden states)
    x_est = [testData.startHandPos(1); testData.startHandPos(2); avgVelocity; zeros(2,1)];
    P_est = eye(6); % Updated size to match hidden states
    ...
    % Predict the angle using Naive Bayes classifier
    ...
    % Kalman filter update with hidden states
    X_pred = A * x_est;
    P_pred = A * P_est * A' + W;
